...
export async function initBook(root, opts = {}) {
  ...
  function renderQuote(q, available) {
    ...
    quoteBox.innerHTML = `
      ...
      <div class="bk-line"><span>${esc(String(q.nights))} night${q.nights === 1 ? '' : 's'}${q.variableNightly ? '' : ' × ' + money(q.nightly, q.currency)}</span><span>${money(q.subtotal, q.currency)}</span></div>
      ${q.variableNightly ? `<div class="bk-line bk-later"><span>Averages ${money(q.avgNightly, q.currency)}/night</span><span></span></div>` : ''}
